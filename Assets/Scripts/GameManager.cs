...
public class GameManager : Singleton<GameManager> {
	...
	readonly Dictionary<(int, int), MapData> mapTest = new();
	enum BlockType {
		dirt = 1,
		water,
		grass,
				...
				for (int k = 0; (k <= h || k <= seaLevel) && k < 256; k++) {
					if (k < h) {
						...
						if (!IsGenerateCave(x, k, z)) {
							d.position[i, k, j] = GenerateBlockTest(k, h);
						}
					}
	...
	float Worley2D(Vector2 uv, bool revert) {
		float Dist = 16.0f;
		Vector2 intPos = uv.Floored();
		Vector2 fracPos = uv.Fractioned();
		//search range
		const int Range = 2;
	...
	Vector2 hash2d2(Vector2 v) {
		...
		int y = (int)v.y;
		return new Vector2(
			hash2d2(x * 193 + y * 241 + 1013) / 1073741824.0f,
			hash2d2(x * 331 + y * 337 + 977) / 1073741824.0f
		);
	}
	/// <summary>
	/// 判断返回草地还是岩石
	...
	float ShouldGenerateTree(int x, int y, int z) {
	...
	static readonly List<Vector2> offPosition = new() {
		new Vector2( 0,  1),
		new Vector2( 0, -1),
		new Vector2( 1,  0),
		new Vector2(-1,  0),
	...
	void CreateAtlas() {
		...
		textures = new() {
			Resources.Load<Texture2D>("Blocks/dirt"),
			Resources.Load<Texture2D>("Blocks/dirt"),
			Resources.Load<Texture2D>("Blocks/dirt"),

			Resources.Load<Texture2D>("Blocks/water_overlay"),
			Resources.Load<Texture2D>("Blocks/water_overlay"),
			Resources.Load<Texture2D>("Blocks/water_overlay"),

			Resources.Load<Texture2D>("Blocks/grass_top"),
			Resources.Load<Texture2D>("Blocks/grass_side"),
			Resources.Load<Texture2D>("Blocks/grass_side"),

			Resources.Load<Texture2D>("Blocks/cobblestone"),
			Resources.Load<Texture2D>("Blocks/cobblestone"),
			Resources.Load<Texture2D>("Blocks/cobblestone"),

			Resources.Load<Texture2D>("Blocks/snow"),
			Resources.Load<Texture2D>("Blocks/grass_side_snowed"),
			Resources.Load<Texture2D>("Blocks/dirt"),

			Resources.Load<Texture2D>("Blocks/ice"),
			Resources.Load<Texture2D>("Blocks/ice"),
			Resources.Load<Texture2D>("Blocks/ice"),

			Resources.Load<Texture2D>("Blocks/log_oak_toplog_oak_top"),
			Resources.Load<Texture2D>("Blocks/log_oak"),
			Resources.Load<Texture2D>("Blocks/log_oak_toplog_oak_top"),

			Resources.Load<Texture2D>("Blocks/leaves_oak"),
			Resources.Load<Texture2D>("Blocks/leaves_oak"),
			Resources.Load<Texture2D>("Blocks/leaves_oak"),
		};
	...
	static readonly Vector3[][] _faceVertices = new Vector3[][] {
		new Vector3[] {
			new(-0.5f, 0.5f, -0.5f),
			new(-0.5f, 0.5f, 0.5f),
			new(0.5f, 0.5f, 0.5f),
	...
	Vector3[] GetFaceVertices(Face face) => _faceVertices[(int)face];
	//TODO 水的双面材质
	/// <summary>
	...
	/// <param name="y"></param>
	/// <param name="z"></param>
	/// <param name="isTransparentBlock">要检测的方块是否是透明方块</param>
	/// <returns>true为显示false为不显示</returns>
	public bool IsVisible(int x, int y, int z, bool isTransparentBlock = false) {
		...
		if (d.position[x, y, z] != 0) {
			...
			return !isTransparentBlock && d.position[x, y, z] == 2;
		}
		return true;
	}
	/// <summary>
